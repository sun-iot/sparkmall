...
import com.sun.bigdata.sparkmall.common.model.{CityInfo, ProductInfo, UserInfo, UserVisitAction}
import com.sun.bigdata.sparkmall.common.util.ConfigUtils
import com.sun.bigdata.sparkmall.mock.date.{RanOpt, RandomDate, RandomNum, RandomOptions}
    ...
    for (i <- 1 to sessionNum) {
      ...
      while (!isQuit) {
        ...
        if (action == "quit") {
          ...
          val cityId: Long = RandomNum(1, 26).toLong
